	...
	function (committeeId) {
		...
		Committee.getCommittee.basic <- 
			function (.committeeId) {
				...
				output  <-  lapply(output, FUN=function(i){ 
					...
					else {data.frame(i,"committeeId" =.committeeId, stringsAsFactors = FALSE)}})
		...
		output.list2 <- lapply(nodes,FUN=function(y) {lapply(output.list, FUN=function(x) {
			...
			if (!is.null(df.)){
				...
				df. <- data.frame(df.,"committeeId"=candId, stringsAsFactors = FALSE)
				df.
			}
		})})
			...
			ol2 <- lapply(ol, function(x){
				...
				if (is.null(x)) {NA}
					...
					if (ncol(x) < max.cols) {
						...
						x[,"committeeId"] <- NULL
						x <- data.frame(cbind(x,matrix(NA, ncol=max.cols-ncol(x)-1, nrow = 1)),
										"committeeId"=candId,row.names=NULL, stringsAsFactors = FALSE)
		...
		names(output) <- gsub(pattern=".text", replacement="", x=names(output), fixed=TRUE)
