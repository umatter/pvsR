...
##' Use District.getByOfficeState() or District.getByZip() to get a list of district ID(s).\cr
...
Candidates.getByDistrict <-
	function (districtId, electionYear=NULL) {
		if (length(electionYear)==0) {
			# internal function
			Candidates.getByDistrict.basic1 <- 
				function (.districtId) {
			...
			# internal function
			Candidates.getByDistrict.basic2 <- 
				function (.districtId, .electionYear) {
					request <-  "Candidates.getByDistrict?"
