...
##' @usage CandidateBio.getBio(candidateId, separate=FALSE)
##' @param candidateId a character string or list of character strings with the candidate ID(s) (see references for details)
##' @param separate logical, indicates whether all subnodes should be returned separately (default:FALSE, returns one data frame)
...
CandidateBio.getBio <-
	function (candidateId, separate=FALSE) {
		...
		CandidateBio.getBio.basic <- function (.candidateId) {
			...
			output  <-  lapply(output, FUN=function(i){ 
				...
				else {data.frame(i,"candidateId" =.candidateId, stringsAsFactors = FALSE)}})
			output
		...
		output.list2 <- lapply(nodes,FUN=function(y) {lapply(output.list, FUN=function(x) {
			...
			if (!is.null(df.)){
				...
				df. <- data.frame(df.,"candidateId"=candId, stringsAsFactors = FALSE)
				df.
			}
		})})
...
# 						x[,"candidateId"] <- NULL
# 						x <- data.frame(cbind(x,matrix(NA, ncol=max.cols-ncol(x)-1, nrow = 1 )),"candidateId"=candId,row.names=NULL, stringsAsFactors = FALSE)
...
# 		})
		...
		if (separate) {
			names(output.list3) <- nodes
			for (i in 1:length(nodes)) {
				node.i <- nodes[i]
				names(output.list3[[i]]) <- gsub(paste0(node.i,"."), "", names(output.list3[[i]]), fixed = TRUE)
			}
			return(output.list3)
			
		} else {
			output <- Reduce(function(x,y) {merge(x,y,  all.x=TRUE)}, output.list3)
			output <- as.tbl(output, stringsAsFactors=FALSE)
			return(output)
		}
	}
